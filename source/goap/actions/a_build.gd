...
func setup():
	...
	cost = 3.0 # Higher cost actions are considered less preferable over lower cost actions when planning
	name = "a_build" # String identifier of this action. setup_common() will make sure the action's node is named after this
	...
	add_precondition("s_has_cargo", true)
	add_precondition("s_has_target", true)

	# Effects - use add_effect(string symbol, bool value)
	add_effect("s_no_constraction_sites", true)
...
func evaluate():
	...
	return !Globals.constructionSites.empty()
