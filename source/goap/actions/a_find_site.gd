...
func setup():
	...
	name = "a_find_site" # String identifier of this action. setup_common() will make sure the action's node is named after this
...
func execute():
	...
	if !Globals.constructionSites.empty():
		entity.target = Globals.constructionSites[ randi() % Globals.constructionSites.size() ]
		return COMPLETED
