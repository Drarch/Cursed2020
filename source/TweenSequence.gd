...
func _init(root: Node = Globals.get_tree().root) -> void:
	__tree = Globals.get_tree()
	__tween = Tween.new()
	__tween.set_meta("sequence", self)
	root.call_deferred("add_child", __tween)
	
	__tween.connect("ready", self, "start", [], CONNECT_ONESHOT)
	__tween.connect("tween_all_completed", self, "__step_complete")
	__tween.connect("tree_entered", __tween, "connect", ["tree_entered", __tween, "resume_all"], CONNECT_ONESHOT)
