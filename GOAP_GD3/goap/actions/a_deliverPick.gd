...
func setup():
	...
	name = "a_deliverPick" # String identifier of this action. setup_common() will make sure the action's node is named after this
	...
	add_precondition("s_hasPick", true)
	
	# Effects - use add_effect(string symbol, bool value)
	add_effect("s_deliveredPick", true)
	return
