...
func setup():
	...
	name = "a_checkTool" # String identifier of this action. setup_common() will make sure the action's node is named after this
...
func evaluate():
	...
	if (entity.get_name() == "Miner"):
		...
		if (entity.hasPick): return true
	
	if (entity.get_name() == "Lumberjack"):
		
		if (entity.hasAxe): return true
