...
func setup():
	...
	name = "g_deliverAxes" # String to identify the goal from others after creation
	...
	add_symbol("s_deliveredAxe", true)
	return
...
func evaluate():
	...
	# Deliver axes if the number of picks is higher
	var pile = get_tree().get_root().get_node("Testscene/Stockpile")
	if (pile.axes <= pile.picks):
		return {"s_deliveredAxe":false}
	return null
