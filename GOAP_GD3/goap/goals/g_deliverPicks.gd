...
func setup():
	...
	name = "g_deliverPicks" # String to identify the goal from others after creation
	...
	add_symbol("s_deliveredPick", true)
	return
...
func evaluate():
	...
	# Prefer delivering picks over axes if number of picks is lower
	var pile = get_tree().get_root().get_node("Testscene/Stockpile")
	if (pile.picks <= pile.axes):
		return {"s_deliveredPick":false}
	return null
